...
const Headword = props => {
    ...
    const {state, setState, addFunctions, moveRow, setScriptForms} = props;
        ...
        <>
            <div className={`row${sectionOpen ? "" : " closed"}`}>
                ...
                {state.entry?.headword?.morphs.map((a,i) => (
                    <Morph state={state} setState={setState} thisIndex={i} key={i} stringPath={pathFragA} prevIndent={0} labels={["Basic form", "Alternate"]}  addFunctions={addFunctions} moveRow={moveRow} setScriptForms={setScriptForms} />
                ))
                }
