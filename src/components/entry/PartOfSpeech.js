...
const PartOfSpeech = props => {
  ...
  const getAllGramForms = () => {
    ...
    gramFormGroups.forEach(a => {
      ...
      let gramFormGroupDef =
        state.setup.gramFormGroups.items.find(b => b.id === a.refId) || state.setup.gramClassGroups.items.find(b => b.id === a.refId);
      let arr = [];
    ...
    <>
      <div className={`row${sectionOpen ? "" : " closed"}`}>
        <div className="row-controls">
          ...
          <i
            className={`fas fa-plus${popupItems.length === 0 ? " disabled" : ""}`}
            onClick={popupItems.length === 0 ? undefined : () => addPopupHandler(addPopupVisible, setAddPopupVisible)}
          ></i>
          ...
          <ul className="parts-of-speech">
            {state.setup.partsOfSpeechDefs.items.map((a, i) => (
              <li
                key={i}
                value={a.id}
                className={isCurrentSelection(a.id) ? "selected" : isAvailable(a.id) ? "" : "disabled"}
        ...
        {path[thisIndex].gramClassGroups?.map((a, i) => (
          <div className="row" key={i}>
            ...
            <div className="row-content" style={getIndent(prevIndent + 1)}>
              ...
              <ul>
                {getGramClasses(path[thisIndex].refId, a.refId, state.setup.partsOfSpeechDefs.items, state.setup.gramClassGroups.items).map(
                  (b, j) => (
                    <li
                      key={j}
                      value={b.id}
                      className={a.gramClasses.some(a => a === b.id) ? "selected" : ""}
                      onClick={e => handleGramClassClick(e, i, a.refId)}
                    >
                      {capitalize(b.name)}
                    </li>
                  )
        ...
        {allGramForms.length > 0 && (
          <GramForms
            path={path}
            cleanUpIrregulars={cleanUpIrregulars}
            state={state}
            setState={setState}
            thisIndex={thisIndex}
            prevIndent={0}
