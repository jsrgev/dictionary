...
const Palette = props => {
  ...
  const path = state.tempSetup.palettes.items;
  const [paletteShown, setPaletteShown] = useState(false);

  return (
    <div className={`${paletteShown ? "palette" : "palette palette-hidden"}`}>
      <div className="show-palette" onClick={() => setPaletteShown(!paletteShown)}>
        {path[thisIndex].name} <i className={paletteShown ? "fas fa-chevron-down" : "fas fa-chevron-up"}></i>
      </div>
        ...
        {path[thisIndex].content.map((a, i) => (
          <React.Fragment key={i}>
            ...
            {a.characters?.map((b, j) => {
              ...
              return (
                <span key={j} style={{ background: a.bgColor, color: a.textColor, gridColumn: gridColumnValue }}>
                  {b}
                </span>
              );
            })}
